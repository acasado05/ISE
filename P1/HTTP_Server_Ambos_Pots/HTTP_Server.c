...
static void clear_LCD_Text (void);
...
//static void Potenciometros (void *arg);
...
//Limpia la primera vez pero no la segunda
//static void clear_LCD_Text(void){
//  for(int i = 0; i < 2; i++){
//    memset(lcd_text[i], '\0', 20+1);
//  }
//}
...
/*----------------------------------------------------------------------------
  Thread 'ADC': Blink the LEDs on an eval board
 *---------------------------------------------------------------------------*/
//static __NO_RETURN void Potenciometros (void *arg) {
//  
...
//  TID_ADC     = osThreadNew (Potenciometros, NULL, NULL);
